...
namespace K_PathFinder.Samples {
    ...
    public class BattleGridUsage : MonoBehaviour {
        ...
        void Update() {
            ...
            if (Vector2.Distance(myPos, _agent.nextPoint.positionV2) < _properties.radius)
                _agent.RemoveNextPoint();

            //it next point still exist then move in that direction with that speed
            if (_agent.haveNextPoint)      
                _controler.SimpleMove(new Vector3(_agent.nextPoint.x - position.x, 0, _agent.nextPoint.z - position.z).normalized * speed);            
        }
