...
namespace K_PathFinder.Rasterization {
    ...
    public class CSRasterization3D {
        ...
        public CSRasterization3DResult Rasterize(Vector3[] verts, int[] tris, Bounds bounds, Matrix4x4 matrix, int volumeSizeX, int volumeSizeZ, float chunkPosX, float chunkPosZ, float voxelSize, float maxSlopeCos, bool debug = false) {
            ...
            voxelBuffer.SetData(new Voxel3D[voxelsTotal]);
            ...
            Voxel3D[] res = new Voxel3D[voxelsTotal];
            voxelBuffer.GetData(res);
            ...
            return new CSRasterization3DResult(res, voxelsX, voxelsY, newVolumeStartX, newVolumeStartZ, newVolumeSizeX, newVolumeSizeZ);
            //return result;

            //float hv = voxelSize * 0.5f;
            //for (int x = 0; x < volume.sizeX; x++) {
            //    for (int z = 0; z < volume.sizeZ; z++) {
            //        if (volume.Exist(x, z)) {
            //            var p = volume.Passability(x, z);
            //            Vector3 mx = new Vector3(chunkPosX + (x * voxelSize) + hv, volume.GetMax(x, z), chunkPosZ + (z * voxelSize) + hv);
            //            Vector3 mn = new Vector3(chunkPosX + (x * voxelSize) + hv, volume.GetMin(x, z), chunkPosZ + (z * voxelSize) + hv);
            //            if (p == Passability.Walkable)
            //                Debuger_K.AddDot(mx, Color.green);
            //            else if (p == Passability.Slope)
            //                Debuger_K.AddDot(mx, Color.cyan);
            //            else
            //                Debuger_K.AddDot(mx, Color.blue);

            //            Debuger_K.AddDot(mn, Color.red);

            //            //Debuger_K.AddLine(mx, mn);
            //        }
            //    }
            //}
        ...
        public CSRasterization3DResult Rasterize(Collider collider, Matrix4x4 matrix, int volumeSizeX, int volumeSizeZ, float chunkPosX, float chunkPosZ, float voxelSize, float maxSlopeCos, bool debug = false) {
            Mesh mesh = collider.gameObject.GetComponent<MeshFilter>().sharedMesh; //TEMP
            Vector3[] verts = mesh.vertices;
            int[] tris = mesh.triangles;

            return Rasterize(verts, tris, collider.bounds, matrix, volumeSizeX, volumeSizeZ, chunkPosX, chunkPosZ, voxelSize, maxSlopeCos, debug);
        }
