...
namespace K_PathFinder.GraphGeneration {    
    public class GraphGenerator {
        private NavMeshTemplateRecast template;
        ...
        public GraphGenerator(VolumeContainer volumeContainer, NavMeshTemplateRecast template) {
        ...
        public Graph MakeGraph() {
            ...
            if (template.doCover) {
                ...
                DouglasPeuckerCover(template.voxelSize * 2, template.voxelSize * 0.5f);
            ...
            if (template.doNavMesh) {
                ...
                RamerDouglasPeucker(template.voxelSize * 0.70f);
        ...
        private void SetGraph(Volume volume, Area targetArea, Passability passability) {
            ...
            for (int x = start; x < endX; x++) {
                for (int z = start; z < endZ; z++) {
                    if(existance[x][z] && //skip if not exist
                        hashMap[x][z] == targetHash && //only target hash can be start point
                        volume.GetState(x, z, VoxelState.MarchingSquareArea) == false && //if we already was here then skip
                        (hashMap[x + 1][z] != targetHash | hashMap[x][z + 1] != targetHash)) { //start iteration if some of plus voxels are not equal to target
        ...
        public void SetCoverGraph(Volume volume) {
            ...
            for (int x = start; x < endX; x++) {
                for (int z = start; z < endZ; z++) {
                    if (existance[x][z] && //skip if not exist
                        coverHashMap[x][z] == targetHash && //only target hash can be start point
                        volume.GetState(x, z, VoxelState.MarchingSquareArea) == false && //if we already was here then skip
                        (coverHashMap[x + 1][z] != targetHash | coverHashMap[x][z + 1] != targetHash)) { //start iteration if some of plus voxels are not equal to target
        ...
        private void RamerDouglasPeucker(float epsilon) {
            ...
            foreach (var node in RDPkeypoints) {
                foreach (var pair in node.getData) {
                    ...
                    if (tempNodes.Count == 2)
                        ...
                        SetupDouglasPeucker(tempNodes, epsilon * 0.25f, epsilon);
                }
            }
        ...
        private void SetupDouglasPeucker(List<NodeTemp> target, float firstEpsilon, float epsilon) {
            NodeTemp A = target[0];
            NodeTemp B = target[1];
            ...
            newValues = DouglasPeucker(newTarget, 0, newTarget.Count - 1, epsilon);
        ...
        private void DouglasPeuckerCover(params float[] epsilonSiquence) {
            #region segments
