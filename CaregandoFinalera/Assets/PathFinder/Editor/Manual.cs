...
namespace K_PathFinder.Instruction {
    public class Manual : EditorWindow {
        ManualData data;
        int sellected = 0;
        string[] s = new string[]{"General", "Agent and Properties", "Results", "Main menu", "Features and Limitations"};
        Vector2 generalScroll, agentAndPropertiesScroll, pathScroll, settingsScroll, featuresAndLimitationsScroll;
        ...
        void OnGUI() {
            ...
            sellected = GUILayout.SelectionGrid(sellected, s, 5);
        ...
        void FeaturesAndLimitations() {
            ...
            GUILayout.Label(data.featuresTexts[2].text);
            GUILayout.Label(data.featuresPics[2], GUILayout.ExpandWidth(false));
            EditorGUILayout.EndScrollView();
        }
    }
}